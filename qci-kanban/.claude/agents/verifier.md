...
3. **Diff review.** Read the actual `git diff` and the changed files from disk. Check, in order of importance:
...
4. **Regression sweep of the test-impact map**: consult `.claude/rules/qci-kanban-test-map.md`; confirm the tests listed for every touched source file were exercised.
...
- Verdict: `APPROVED` only if the full suite passed, the app gate passed (when applicable), and there are zero critical/warning findings. Otherwise `CHANGES-REQUESTED` with the required fixes listed. Declaring premature victory is the single failure mode you exist to prevent.
