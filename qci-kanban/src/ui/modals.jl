...
function _build_edit_form(m::AppModel, iss::Union{Domain.Issue, Nothing})
    ...
    epic_sel =
        iss === nothing || iss.epic_id === nothing ? 1 :
    ...
    sprint_sel =
        iss === nothing || iss.sprint_id === nothing ? 1 :
    ...
    asg_sel =
        iss === nothing || iss.assignee_id === nothing ? 1 :
    ...
    EditForm(
        ...
        _make_input(;
            text = iss === nothing || iss.story_points === nothing ? "" :
                   string(iss.story_points),
        ),
        ...
        _make_input(;
            text = iss === nothing || iss.start_date === nothing ? "" :
                   string(iss.start_date),
        ),
        _make_input(;
            text = iss === nothing || iss.due_date === nothing ? "" : string(iss.due_date),
        ),
...
function _submit_comment!(m::AppModel)
    ...
    Stores.add_comment!(
        m.boardstore;
        issue_id = m.card_issue_id,
        author_id = m.current_user.id,
        body = String(body),
    )
    iss = Stores.get_issue(m.boardstore, m.card_issue_id)
    if iss !== nothing
        ...
        iss.assignee_id === nothing ||
            iss.assignee_id == m.current_user.id ||
...
function _request_bulk_delete!(m::AppModel)
    ...
    m.confirm_kind = :bulk_delete;
    m.confirm_target = targets
    m.modal = :confirm;
    m.focus = FocusState()
    m
end
...
function _open_new_sprint!(m::AppModel)
    set_text!(m.sprint_name_input, "");
    set_text!(m.sprint_goal_input, "")
    m.modal = :new_sprint
...
function _submit_new_sprint!(m::AppModel)
    ...
    if isempty(name)
        m.message = "Sprint name required";
        return m
    end
    s = Stores.create_sprint!(
        m.boardstore;
        name = String(name),
...
function render_card_detail!(m::AppModel, buf::Buffer, content_area::Rect)
    ...
    set_string!(
        ...
        x,
        y,
        _short(iss.title, inner.width - 2),
        Style(; fg = col_text(), bold = true),
    );
    y += 1
    meta =
        "Status:$(iss.status)  Prio:$(iss.priority)  " *
        (iss.story_points === nothing ? "" : "$(iss.story_points)sp  ") *
        "Epic:$(_epic_name(m, iss.epic_id))"
    set_string!(buf, x, y, _short(meta, inner.width - 2), Style(; fg = col_text_dim()));
    y += 1
    asg = iss.assignee_id === nothing ? "Unassigned" : _user_name(m, iss.assignee_id)
    start = iss.start_date === nothing ? "—" : string(iss.start_date)
    due = iss.due_date === nothing ? "—" : string(iss.due_date)
    set_string!(
        buf,
        x,
        y,
        _short("Assignee:$(asg)  Start:$(start)  Due:$(due)", inner.width - 2),
    ...
    if !isempty(iss.description)
        set_string!(
            ...
            x,
            y,
            _short(iss.description, inner.width - 2),
            Style(; fg = col_text()),
        );
        y += 1
    end
    ...
    y += 1;
    y <= maxy && (
        set_string!(buf, x, y, "COMMENTS", Style(; fg = col_primary(), bold = true));
        y += 1
    )
    for c in Stores.list_comments(m.boardstore, iss.id)
        ...
        set_string!(
            ...
            x,
            y,
            _short("• $(who): $(c.body)", inner.width - 2),
            Style(; fg = col_text()),
        );
        y += 1
    end
    ...
    if !isempty(acts) && y <= maxy - 2
        set_string!(buf, x, y, "ACTIVITY", Style(; fg = col_primary(), bold = true));
        y += 1
        for a in acts[max(1, end - 2):end]
            ...
            set_string!(
                ...
                x,
                y,
                _short("· $(a.kind): $(a.detail)", inner.width - 2),
...
function render_card_edit!(m::AppModel, buf::Buffer, content_area::Rect)
    ...
    title =
        creating ? "NEW CARD — Tab fields, ^S/Enter save, Esc cancel" :
        "EDIT CARD — Tab fields, ^S/Enter save"
    ...
    set_string!(
        ...
        y,
        "Title:",
        f.title_input.focused ? Style(; fg = col_primary(), bold = true) :
        Style(; fg = col_text_dim()),
    )
    render(f.title_input, Rect(x + 7, y, iw - 7, 1), buf);
    y += 1
    set_string!(
        buf,
        x,
        y,
        "Desc:",
        f.desc_area.focused ? Style(; fg = col_primary(), bold = true) :
        Style(; fg = col_text_dim()),
    )
    render(f.desc_area, Rect(x + 7, y, iw - 7, 2), buf);
    y += 2
    render(f.priority_sel, Rect(x, y, iw, 1), buf);
    y += 1
    set_string!(
        buf,
        x,
        y,
        "Points:",
        f.points_input.focused ? Style(; fg = col_primary(), bold = true) :
        Style(; fg = col_text_dim()),
    )
    render(f.points_input, Rect(x + 8, y, 8, 1), buf);
    y += 1
    render(f.epic_sel, Rect(x, y, iw, 1), buf);
    y += 1
    render(f.sprint_sel, Rect(x, y, iw, 1), buf);
    y += 1
    render(f.assignee_sel, Rect(x, y, iw, 1), buf);
    y += 1
    set_string!(
        buf,
        x,
        y,
        "Start:",
        f.start_input.focused ? Style(; fg = col_primary(), bold = true) :
        Style(; fg = col_text_dim()),
    )
    ...
    set_string!(
        ...
        y,
        "Due:",
        f.due_input.focused ? Style(; fg = col_primary(), bold = true) :
        Style(; fg = col_text_dim()),
    )
...
function render_confirm!(m::AppModel, buf::Buffer, content_area::Rect)
    msg =
        m.confirm_kind === :bulk_delete ?
        "Delete $(length(m.confirm_target)) selected issues?" :
        m.confirm_kind === :close_sprint ?
        "Close sprint? Incomplete issues roll back to backlog." : "Delete this issue?"
...
function render_search!(m::AppModel, buf::Buffer, content_area::Rect)
    inner = _modal_box(
        content_area,
        clamp(50, 24, content_area.width),
        5,
...
function render_new_sprint!(m::AppModel, buf::Buffer, content_area::Rect)
    inner = _modal_box(
        ...
        clamp(50, 24, content_area.width),
        7,
        "NEW SPRINT — Tab, Enter create, Esc cancel",
        buf,
    )
    x = inner.x + 1;
    y = inner.y + 1
    set_string!(
        ...
        y,
        "Name:",
        m.sprint_name_input.focused ? Style(; fg = col_primary(), bold = true) :
        Style(; fg = col_text_dim()),
    )
    render(m.sprint_name_input, Rect(x + 6, y, inner.width - 7, 1), buf);
    y += 2
    set_string!(
        buf,
        x,
        y,
        "Goal:",
        m.sprint_goal_input.focused ? Style(; fg = col_primary(), bold = true) :
        Style(; fg = col_text_dim()),
    )
